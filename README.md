...
- Secure mode escapes output with `htmlspecialchars`.

### 2) SQL Injection

- Vulnerable mode concatenates user input into SQL.
- Secure mode uses prepared statements with bound parameters.

### 3) Cross-Site Request Forgery (CSRF)

- Vulnerable mode accepts state-changing POST without CSRF token validation.
- Secure mode verifies a session-bound token via `hash_equals`.
